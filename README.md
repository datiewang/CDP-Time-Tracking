...
[azure_registration_guide.md](azure_registration_guide.md) の手順に従って Azure Portal で新しいクライアント シークレットを**生成**し、`graph_config.json` の `"client_secret"` の項目に**手動で入力**してください（シークレットの値は生成直後しか表示されないため、新環境では毎回新しく生成する必要があります）。
